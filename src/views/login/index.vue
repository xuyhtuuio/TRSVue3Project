<template>
  <div class="unity-login">
    <div class="main">
        ...
        <div class="container-top" :class="crtOp === 'register' ? 'pt52' : 'pt80'">
          ...
          <div class="right" v-if="crtOp === 'inputPhone'">
            ...
            <div>
              ...
              <!-- 下一步 -->
              <el-button link class="next" @click="next">下一步</el-button>
          ...
          <div class="right" v-if="crtOp === 'inputPsw'">
            ...
            <div class="pwd-login">
              <el-button link class="nextPwd" @click="next" :loading="loading" :disabled="loading">登录</el-button>
              <!-- 服务协议 -->
            </div>
          </div>
          <div class="right" v-if="crtOp === 'inputSms'">
            ...
            <p class="input-code-warn">
              请输入发送至
              <span>{{ phoneFormat }}</span> 的 <span>6</span> 位验证码， 有效期 5 分钟
            </p>
            ...
            <el-button @click="login" class="login" link>登录</el-button>
          </div>
          ...
          <div class="right" v-if="crtOp === 'forget-pwd'">
            ...
            <div class="forget_pwd">
              ...
              <el-button link @click="submit" class="next edit_pwd">提交</el-button>
            </div>
          </div>
        </div>
      ...
      >
        <template #header>
          <div class="title"><img src="@/assets/image/tips.png" />提示</div>
        </template>
        <div class="tips">
          请先同意
          <span>用户协议</span> 和
        ...
        <template #footer>
          <span class="dialog-footer">
            <el-button link class="cancel" @click="checkAgreement = false">取 消</el-button>
            <el-button link class="submitCheck" @click="checkAgreement = false"
              >确 定</el-button
            >
          </span>
        </template>
      ...
      >
        ...
        <template #footer>
            <span class="dialog-footer">
            <el-button link class="cancel" @click="findPwd = false">取 消</el-button>
            <el-button link class="submitCheck" @click="findPwd = false">确 定</el-button>
          </span>
        </template>
      </el-dialog>
      <el-dialog
      ...
      >
        ...
        <template #footer>
            <span class="dialog-footer">
            <el-button link class="cancel" @click="applicationCancel">取消</el-button>
            <el-button link class="submitCheck" @click="applicationExpiry">申请延期</el-button>
          </span>
        </template>
      </el-dialog>
      <!-- 提交申请成功 -->
      <el-dialog
        v-model="applySuccessdialog"
        width="600px"
        class="applyDialog"
        :before-close="handleClose"
      >
...
export default {
  ...
  computed: {
    phoneFormat() {
      return this.form.phone ? this.form.phone.substring(0, 3) + '****' + val.substring(7) : '';
    }
  },
  methods: {
    findMyPwd() {
        ...
        this.$refs['form'].validate(async valid => {
          if (valid) {
            ...
            try {
              ...
              this.loading = false
              if (!res.access_token) {
                ElMessage.error(res?.error_description);
                this.codeFun();
                return;
              }
              if (res.access_token) {
                ...
                if (this.gohistory && this.gohistory !== '404') {
                  ...
                  this.$router.push({ name: 'complaintEntry' });
                }
                // this.sendMessage = {
              ...
              if (err && err.data && err.data.error === 'invalid_grant') {
                _ElMessage.error(err.data.error_description);
              } else {
                _ElMessage.error(err.data.error);
              }
              _this.form.verify = '';
              _this.codeFun();
            }
    ...
    // 系统内部手机号验证码
    async getCodeMible() {
      return ElMessage.success('当前只能用账号密码登录');
    ...
    login() {
      ...
      this.code = code;
      if (this.code.length !== 6) return ElMessage.error('请输入短信验证码');
      this.loginByMible();
    },
    async loginByMible() {
      const _this = this;
      try {
        ...
        if (res.access_token) {
          ...
          return ElMessage.error(res.error_description);
        }
      } catch (err) {
        if (
  ...
  .container-top {
    ...
    .right {
      ...
      :deep(.el-form) {
        .el-input-group {
          height: 50px;
          border: 1px solid rgb(202,205,211);
          border-radius: 4px;
          .el-input__wrapper {
            border: none;
            box-shadow: none;
