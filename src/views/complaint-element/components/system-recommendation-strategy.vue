...
<template>
    <div class="system-strategy">
        <p class="title"> <span class="iconfont" style="color: #306ef5;margin-right: 4px;">&#xe625;</span>系统推荐策略</p>
        <div class="strategy">
                ...
                <div class="title" style="justify-content: space-between; align-items: center;">
                    ...
                    <span class="flex pointer" style="align-items: center;">
                        <el-icon style="color: #2D5CF6;">
                            <Refresh />
                        </el-icon> <i class="pointer" style="margin-left: 4px;">换一换</i></span>
                </div>
                <div class="strategy-content">
                    <p class="key-point">
                        应对要点：
                    </p>
                    <p class="point-value">听取客户反馈；确认理解；道歉；提供修改；再次确认；确认解决方案；结束</p>
                    <p class="key-point">
                        推荐处理意见书：
                    </p>
                    <div class="opinions">
            ...
            <div class="strategy-box">
                ...
                <div class="strategy-content" v-for="(item, index) in similarData" :key="index">
                    ...
                    <div :class="{ 'case-value pointer': true, 'ellipsis ellipsis_2': item.fold }"
...
onMounted(() => {
    ...
    similarData.value = opinion.similarCase.slice(0, 2)
})
let unfold = (key, item, index) => {
    if (key == 'suggestion') {
        suggestionFold.value ? suggestions.value = opinion.opinions.slice(0, 1) : suggestions.value = opinion.opinions
    ...
    if (key == 'similar') {
        similarData.value = similarFold.value ? opinion.similarCase.slice(0, 2) : similarData.value.concat(opinion.similarCase.slice(2))
